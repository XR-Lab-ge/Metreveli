...
float3 calculateSpecular(in float4 albedo)
{
    ...
    if (float(2) == 1) // Realistic
    {
        ...
        finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(1,1,1,1), metallic, float(0.434) * specularMap.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
    }
    ...
    if (float(2) == 4)
    {
        ...
        if (float(2) == 2) // Toon
        {
            finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.3649385,0.5827802,0,1), metallic, float(1), specularMap, specularLightColor, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
        ...
        if (float(2) == 3) // anisotropic
        {
            ...
            jitter += float(0);
            float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
            finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.434), float(0), float(1), float(1), float4(1,1,1,1), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(1,1,1,1);
            finalSpecular *= attenuation;
        }
    #endif
        ...
        for (int index = 0; index < 4; index++)
        {
            ...
            if (float(2) == 1) // Realistic
            {
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(1,1,1,1), metallic, float(0.434) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
            }
        }
    #endif
