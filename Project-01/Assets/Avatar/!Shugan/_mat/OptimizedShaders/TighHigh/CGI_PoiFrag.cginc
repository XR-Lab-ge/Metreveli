...
float4 frag(v2f i, uint facing: SV_IsFrontFace): SV_Target
{
	...
	**********************************************************************/
	...
	#endif
	float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, TRANSFORM_TEX(poiMesh.uv[float(2)], _MainTex) + _Time.x * float4(0,0,0,0));
	half3 detailMask = 1;
	...
	**********************************************************************/
		...
		#ifdef POI_SPECULAR
			meta.SpecularColor = poiLight.color.rgb * float4(0.957614,0.957614,0.957614,1).rgb * lerp(1, albedo.rgb, float(0.986)) * float4(0.957614,0.957614,0.957614,1).a;
		#else
