...
float3 calculateSpecular(in float4 albedo)
{
    ...
    if (float(1) == 1) // Realistic
    {
        ...
        finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.7799658,0.6109761,0.6021242,1), metallic, float(0.282) * specularMap.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
    }
    ...
    if (float(1) == 4)
    {
        ...
        finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.95), float(1), float4(0.7799658,0.6109761,0.6021242,1), albedo, metallic, jitter, float(0), packedTangentMap);
        ...
        if (float(1) == 2) // Toon
        {
            ...
            finalSpecular *= float4(0.7799658,0.6109761,0.6021242,1);
        }
        ...
        if (float(1) == 3) // anisotropic
        {
            ...
            jitter += float(0);
            float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
            finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.282), float(0), float(1), float(1), float4(0.7799658,0.6109761,0.6021242,1), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(0.7799658,0.6109761,0.6021242,1);
            finalSpecular *= attenuation;
        }
    #endif
        ...
        for (int index = 0; index < 4; index++)
        {
            ...
            if (float(1) == 1) // Realistic
            {
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.7799658,0.6109761,0.6021242,1), metallic, float(0.282) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
            }
        }
    #endif
    finalSpecular *= float4(0.7799658,0.6109761,0.6021242,1).a;
