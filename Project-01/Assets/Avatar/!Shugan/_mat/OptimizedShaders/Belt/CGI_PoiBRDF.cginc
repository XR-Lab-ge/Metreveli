...
void poiBRDF(inout float4 finalColor, const float4 finalColorBeforeLighting)
{
    ...
    float metallic = metallicGlossMap.r * float(0.4);
    float reflectance = metallicGlossMap.g * float(0.368);
    float roughness = max(1 - (float(0.247) * metallicGlossMap.a), getGeometricSpecularAA(poiMesh.normals[1]));
