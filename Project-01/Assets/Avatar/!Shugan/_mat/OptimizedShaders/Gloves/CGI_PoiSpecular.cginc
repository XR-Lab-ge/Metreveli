...
float3 calculateSpecular(in float4 albedo)
{
    ...
    half metallic = POI2D_SAMPLER_PAN(_SpecularMetallicMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).r * float(0.932);
    ...
    if (float(0.41))
    {
        specularLightColor = clamp(poiLight.color, 0, float(0.41));
    }
    #ifdef FORWARD_ADD_PASS
    ...
    if (float(2) == 4)
    {
        ...
        if (float(2) == 2) // Toon
        {
            finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0,0.2245553,0,1), metallic, float(1), specularMap, specularLightColor, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
