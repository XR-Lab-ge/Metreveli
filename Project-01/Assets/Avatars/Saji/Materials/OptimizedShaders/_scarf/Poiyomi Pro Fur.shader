Shader "Hidden/Locked/.poiyomi/Poiyomi Pro Fur/aa4718311c3901842b94029755ab2172"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			)
			{
				...
				{
					distanceOffset *= lerp(1.0, clamp((distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, o.localPos).xyz)), 0.0f, (0.27 /*_OutlinesMaxDistance*/)), (0.477 /*_OutlineFixWidth*/));
				}
			...
			void GeometryProgram(triangle VertexOut p[3], inout TriangleStream<VertexOut> tristream, uint InstanceID : SV_GSInstanceID)
			{
				...
				uint layerCount = max(1, ceil((4 /*_FurLayerCount*/) * (1 - smoothstep((2.0 /*_FurMinDistance*/), (8.0 /*_FurMaxDistance*/), distance(objectPos, getCameraPosition())))));
				...
				for (int i = 0; i < 3; i++)
				{
					...
					float3 furGravityOffset = (float3(0, -1, 0) * (furSettings.y * (0.0047 /*_FurLength*/)) * (0.0 /*_FurGravityStrength*/)) * smoothstep(-0.5, 1, dot(normal, float3(0, -sign((0.0 /*_FurGravityStrength*/)), 0)) * 0.5 + 0.5);
					t.furSettings = furSettings;
					float3 offset = furGravityOffset +furSettings.y * (0.0047 /*_FurLength*/) * normal;
			...
			void applyFur(in float2 furSettings, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				furHeightMap = saturate(remap(furHeightMap, 0, float4(0.2157613,0.8582112,0,1).y, float4(0.2157613,0.8582112,0,1).x, 1));
				float ao = lerp(1, smoothstep(float4(0,0.9999999,0,1).x, float4(0,0.9999999,0,1).y, furHeightMap), (0.355 /*_FurAO*/) * furMask);
				float4 furTexture = POI2D_SAMPLER_PAN(_FurTexture, _MainTex, poiUV(poiMesh.uv[(0.0 /*_FurTextureUV*/)], float4(12,12,0,0.09)), float4(0,0,0,0));
			...
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DetailNormalMapUV*/)], float4(30,30,0,0)), float4(0,0,0,0), (0.0 /*_DetailNormalMapStochastic*/)), (0.25 /*_DetailNormalMapScale*/) * poiMods.detailMask.g);
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#endif
				float shadowStrength = (0.718 /*_ShadowStrength*/) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, (0.5 /*_ShadowBorder*/), (0.158 /*_ShadowBlur*/) * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, (0.5 /*_ShadowBorder*/), (0.158 /*_ShadowBlur*/) * blurMap.r, (0.0 /*_ShadowBorderRange*/));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, (0.5 /*_ShadowBorder*/), (0.158 /*_ShadowBlur*/) * blurMap.r);
					...
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, (0.5 /*_ShadowBorder*/), (0.158 /*_ShadowBlur*/) * blurMap.r, (0.0 /*_ShadowBorderRange*/));
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (float4(0.07698169,0.08839308,0.1807606,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.07698169,0.08839308,0.1807606,1).rgb;
				}
				...
				indirectColor = lerp(indirectColor, 1, lns.w * float4(0.2411783,0.2411783,0.2411783,1).rgb * float4(0.2411783,0.2411783,0.2411783,1).a);
			...
			float2 UnitSquareToUnitDiskPolar(float a, float b)
			{
				...
				{
					radius = a;
					angle = (PI_OVER_4) * (b / (a + EPSILON));
				}
				else
				{
					radius = b;
					angle = PI_OVER_2 - (PI_OVER_4) * (a / (b + EPSILON));
				}
				if (radius < 0)
				{
					radius *= -1.0f;
			...
			float2 HammersleySequence(int i, int N)
			{
			...
			float2 HaltonSequence(int index)
			{
			...
			{
				float ao = 0.0;
				int totalSamples = int((10.0 /*_SSAOQuality*/) * AO_DIRECTIONS);
				float centerImportance = (1.0 /*_SSAOCenterImportance*/);
				float radiusFactor = radius * rcp(poiCam.clipPos.w);
				float maxDepthDifference = radius * 3;
				float totalWeight = 0;
				float3 tangent, bitangent;
				float isYUp = step(0.999, abs(normal.y));
				float3 up = lerp(float3(0, 1, 0), float3(1, 0, 0), isYUp);
				...
				{
					float2 screenOffset;
					if ((0.0 /*_SSAOSamplingMethod*/) == 0.0)
					{
						int i = sampleIndex / int((10.0 /*_SSAOQuality*/));
						int j = sampleIndex % int((10.0 /*_SSAOQuality*/));
						float invQualityMinusOne = 1.0 / ((10.0 /*_SSAOQuality*/) - 1);
						float distance = float(j) * invQualityMinusOne;
						...
						offsetFactor = distance * lerp(1.0, distance, centerImportance);
						screenOffset = sampleDir.xy * radiusFactor * offsetFactor;
					}
					else if ((0.0 /*_SSAOSamplingMethod*/) == 1.0)
					{
						float2 uvSample = float2(
						frac(float(sampleIndex) * 0.61803398875) * 2.0f - 1.0f,
						frac(float(sampleIndex) * 0.32450996011) * 2.0f - 1.0f
						);
						float2 sampleOffset = SquareToDiskMapping(uvSample.x, uvSample.y);
						offsetFactor = length(sampleOffset) * lerp(1.0, length(sampleOffset), centerImportance);
						screenOffset = (tangent.xy * sampleOffset.x + bitangent.xy * sampleOffset.y) * radiusFactor * offsetFactor;
					}
					else if ((0.0 /*_SSAOSamplingMethod*/) == 2.0)
					{
						float2 randomOffset = float2(
						frac(sin(dot(float2(sampleIndex, sampleIndex + 1), float2(12.9898, 78.233))) * 43758.5453),
						frac(sin(dot(float2(sampleIndex + 2, sampleIndex + 3), float2(12.9898, 78.233))) * 43758.5453)
						);
						float angle = randomOffset.x * TWO_PI;
						float distance = sqrt(randomOffset.y);
						float s, c;
						sincos(angle, s, c);
						float2 sampleOffset = float2(c, s) * distance;
						offsetFactor = distance * lerp(1.0, distance, centerImportance);
						screenOffset = (tangent.xy * sampleOffset.x + bitangent.xy * sampleOffset.y) * radiusFactor * offsetFactor;
					}
					else if ((0.0 /*_SSAOSamplingMethod*/) == 3.0)
					{
						float2 hammersley = HammersleySequence(sampleIndex, totalSamples);
						float angle = hammersley.x * TWO_PI;
						float distance = hammersley.y;
						float s, c;
						sincos(angle, s, c);
						float2 sampleOffset = float2(c, s) * distance;
						offsetFactor = distance * lerp(1.0, distance, centerImportance);
					...
					else if ((0.0 /*_SSAOSamplingMethod*/) == 4.0)
					{
						float angle = float(sampleIndex) * 0.5;
						float distance = float(sampleIndex) / totalSamples;
						float s, c;
						sincos(angle, s, c);
						float2 sampleOffset = float2(c, s) * distance;
						offsetFactor = distance * lerp(1.0, distance, centerImportance);
					...
					else if ((0.0 /*_SSAOSamplingMethod*/) == 5.0)
					{
						float2 halton = HaltonSequence(sampleIndex + 1); // Halton sequence starts at index 1
						float angle = halton.x * TWO_PI;
						float distance = halton.y;
						float s, c;
						sincos(angle, s, c);
						float2 sampleOffset = float2(c, s) * distance;
						offsetFactor = distance * lerp(1.0, distance, centerImportance);
					...
					else if ((0.0 /*_SSAOSamplingMethod*/) == 6.0)
					{
						float2 poissonDiskSamples[12] = {
						...
						int idx = sampleIndex % 12;
						float2 sampleOffset = poissonDiskSamples[idx] * (float(sampleIndex) / totalSamples);
						offsetFactor = length(sampleOffset) * lerp(1.0, length(sampleOffset), centerImportance);
						screenOffset = (tangent.xy * sampleOffset.x + bitangent.xy * sampleOffset.y) * radiusFactor * offsetFactor;
					}
					else
					...
					float2 samplePos = clamp(uv + screenOffset, 0.0, 1.0);
					...
					aoValue = smoothstep(float4(0.002,0.007,0.01,0.5).x, float4(0.002,0.007,0.01,0.5).y, depthDifference);
					aoValue *= smoothstep(float4(0.002,0.007,0.01,0.5).w, float4(0.002,0.007,0.01,0.5).z, depthDifference);
					ao += aoValue;
				}
				return 1.0 - saturate((ao / totalSamples) * (1.0 /*_SSAOIntensity*/));
			}
			float calculateSSAO(PoiMesh poiMesh, PoiCam poiCam, inout PoiLight poiLight)
			{
				float perspectiveDivide = rcp(poiCam.clipPos.w);
				...
				float3 transformedNormal = mul((float3x3)UNITY_MATRIX_V, poiMesh.normals[0]);
				#if defined(PROP_SSAOMASK) || !defined(OPTIMIZER_ENABLED)
				float mask = POI2D_SAMPLER_PAN(_SSAOMask, _MainTex, poiUV(poiMesh.uv[(0.0 /*_SSAOMaskUV*/)], float4(1,1,0,0)), float4(0,0,0,0))[(0.0 /*_SSAOMaskChannel*/)];
				if ((0.0 /*_SSAOMaskInvert*/) > 0)
				mask = 1 - mask;
				#else
				float mask = 1;
				#endif
				float ssao = CalculateAmbientOcclusion(screenPos, depth, transformedNormal, (0.005 /*_SSAORadius*/), direction, poiMesh, poiCam);
				return lerp(1, ssao, mask);
			}
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods)
			{
				float3 ssaoColor = poiThemeColor(poiMods, float4(0,0,0,1).rgb, (0.0 /*_SSAOColorThemeIndex*/)).rgb * lerp(1, poiFragData.finalColor, (0.0 /*_SSAOUseSurfaceColor*/));
				#if defined(PROP_SSAOCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				ssaoColor *= POI2D_SAMPLER_PAN(_SSAOColorMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_SSAOColorMapUV*/)], float4(1,1,0,0)), float4(0,0,0,0));
				#endif
				poiFragData.finalColor = lerp(poiFragData.finalColor, customBlend(poiFragData.finalColor, ssaoColor, (2.0 /*_SSAOBlendType*/)), 1 - ssao);
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[(0.0 /*_MainTexUV*/)].xy, float4(12,12,0,0.09));
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_BumpMapUV*/)].xy, float4(3,12,1.7,1.07)), float4(0,0,0,0), (0.0 /*_BumpMapStochastic*/)), (0.5 /*_BumpScale*/));
				#else
				...
				#if defined(POI_SSAO)
				float ssao = calculateSSAO(poiMesh, poiCam, poiLight);
				#endif
				...
				#endif
				#if defined(POI_SSAO)
				applySSAO(ssao, poiMesh, poiFragData, poiMods);
				#endif
				if ((0.0 /*_IgnoreFog*/) == 0)
				{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				...
				if ((0.0 /*_OutlineAlphaDistanceFade*/))
				{
					...
					poiFragData.alpha *= lerp((0.0 /*_OutlineAlphaDistanceFadeMinAlpha*/), (1.0 /*_OutlineAlphaDistanceFadeMaxAlpha*/), smoothstep((0.0 /*_OutlineAlphaDistanceFadeMin*/), (1.16 /*_OutlineAlphaDistanceFadeMax*/), distance(position, poiCam.worldPos)));
				}
