...
UnityIndirect CreateIndirectLight(float3 normal)
{
    ...
    indirectLight.specular = 0;
        ...
        indirectLight.diffuse = max(indirectLight.diffuse, float(0.35));
...
void calculateBasePassLightMaps()
{
        ...
        shadowStrength = 1;
        #ifndef OUTLINE
            shadowStrength = POI2D_SAMPLER_PAN(_LightingShadowMask, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float(0.1);
        #else
        ...
        directLighting = lerp(directLighting, dot(directLighting, float3(0.299, 0.587, 0.114)), float(0.5));
        indirectLighting = lerp(indirectLighting, dot(indirectLighting, float3(0.299, 0.587, 0.114)), float(0.5));
        ...
        switch(float(0))
        {
            ...
            case 1: // Math Gradient
            {
                ...
                if (float(0))
                {
                    poiLight.finalLighting = lerp((directLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.1)));
                }
                else
                {
                    poiLight.finalLighting = lerp((indirectLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.1)));
                }
            }
            break;
            case 2:
        ...
        if (float(0) == 2) // Wrapped
        {
            ...
            poiLight.finalLighting = (poiLight.indirectLighting + poiLight.directLighting) * saturate(poiLight.rampedLightMap + 1 - float(0.1));
        }
        if (!float(0))
        {
        ...
        switch(float(0))
        {
            ...
            case 2: // or wrapped
            {
                ...
                if (float(0.35) > 0)
                {
                    ...
                    finalLighting = max(poiLight.finalLighting, poiLight.finalLighting / max(0.0001, (finalluminance / float(0.35))));
            ...
            case 1: // realistic
            {
                ...
                finalLighting = lerp(realisticLighting, dot(realisticLighting, float3(0.299, 0.587, 0.114)), float(0.5));
            }
            break;
            case 3: // Skin
            {
