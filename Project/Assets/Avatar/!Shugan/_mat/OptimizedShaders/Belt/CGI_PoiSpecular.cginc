...
float3 calculateSpecular(in float4 albedo)
{
    ...
    if (float(-0.02))
    {
        specularLightColor = clamp(poiLight.color, 0, float(-0.02));
    }
    #ifdef FORWARD_ADD_PASS
    ...
    if (float(2) == 4)
    {
        ...
        if (float(2) == 2) // Toon
        {
            finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.4109216,0.9999999,0,1), metallic, float(1), specularMap, specularLightColor, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
