...
void calculateDissolve(inout float4 albedo, inout float3 dissolveEmission)
{
    ...
    #endif
    #if defined(PROP_DISSOLVENOISETEXTURE) || !defined(OPTIMIZER_ENABLED)
        float dissolveNoiseTexture = POI2D_SAMPLER_PAN(_DissolveNoiseTexture, _MainTex, poiMesh.uv[float(0)], float4(0,0.03,0,0)).r;
    #else
    ...
    if (float(1) == 1) // Basic
    {
        ...
        edgeAlpha = remapClamped(noise, da + float(0.319), da, 0, 1) * (1 - dissolveAlpha);
    }
    ...
    dissolveEmission = lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(0), smoothstep(0, 1 - float(0.731) * .99999999999, edgeAlpha));
}
#endif
