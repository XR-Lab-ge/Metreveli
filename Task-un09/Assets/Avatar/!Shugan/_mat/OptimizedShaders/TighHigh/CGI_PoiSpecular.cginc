...
float3 calculateSpecular(in float4 albedo)
{
    ...
    half metallic = POI2D_SAMPLER_PAN(_SpecularMetallicMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).r * float(0.986);
    ...
    if (float(0.39))
    {
        specularLightColor = clamp(poiLight.color, 0, float(0.39));
    }
    #ifdef FORWARD_ADD_PASS
    ...
    if (float(2) == 1) // Realistic
    {
        ...
        finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.957614,0.957614,0.957614,1), metallic, float(0.434) * specularMap.a, poiLight.dotNH, poiLight.dotLH, specularLightColor, attenuation);
    }
    ...
    if (float(2) == 4)
    {
        ...
        finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.95), float(1), float4(0.957614,0.957614,0.957614,1), albedo, metallic, jitter, float(0), packedTangentMap);
        ...
        if (float(2) == 2) // Toon
        {
            finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.5373523,0.9394984,0,1), metallic, float(1), specularMap, specularLightColor, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
            finalSpecular *= float4(0.957614,0.957614,0.957614,1);
        }
        ...
        if (float(2) == 3) // anisotropic
        {
            ...
            jitter += float(0);
            float4 packedTangentMap = POI2D_SAMPLER_PAN(_AnisoTangentMap, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
            finalSpecular += AnisotropicSpecular(float(0), float(0), float(0.434), float(0), float(1), float(1), float4(0.957614,0.957614,0.957614,1), metallic, specularMap, specularLightColor, poiLight.direction, poiLight.halfDir, poiLight.nDotL, jitter, packedTangentMap, albedo);
            finalSpecular *= float4(0.957614,0.957614,0.957614,1);
            finalSpecular *= attenuation;
        }
    #endif
        ...
        for (int index = 0; index < 4; index++)
        {
            ...
            if (float(2) == 1) // Realistic
            {
                finalSpecular += calculateNewSpecular(specularMap.rgb, float(1), realisticAlbedo, float4(0.957614,0.957614,0.957614,1), metallic, float(0.434) * specularMap.a, poiLight.vDotNH[index], poiLight.vDotLH[index], poiLight.vColor[index], poiLight.vAttenuationDotNL[index]);
            }
        }
    #endif
    finalSpecular *= float4(0.957614,0.957614,0.957614,1).a;
