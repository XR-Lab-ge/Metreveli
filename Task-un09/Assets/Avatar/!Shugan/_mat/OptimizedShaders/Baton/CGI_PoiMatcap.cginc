    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        half2 matcapUV = half2(dot(worldViewRight, poiMesh.normals[float(1)]), dot(worldViewUp, poiMesh.normals[float(1)])) * float(0.455) + 0.5;
        #if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
            matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(1,0.02093114,0.02093114,1);
        #else
            matcap = float4(1,0.02093114,0.02093114,1);
        #endif
        matcap.rgb *= float(1);
        #if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
        ...
        , _BlackLightMaskMatcap
            ...
            blendMatcap(finalColor, float(0.014), float(0.135), float(0), matcap2, matcap2Mask, matcapEmission, float(0)
            #ifdef POI_LIGHTING
            , float(0)
            #endif
            #ifdef POI_BLACKLIGHT
